...
export default function ProjectMembersDialog({ open, onOpenChange, project, currentUser, t }) {
  ...
  const myRole = normalizeRole(project?.role);
  ...
  const loadMembers = React.useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await websiteApi.listProjectMembers(projectId);
  ...
  const inviteMember = async (event) => {
    ...
    setBusyKey("invite");
    try {
      const res = await websiteApi.inviteProjectMember({ projectId, email, role: inviteRole });
  ...
  const updateRole = async (member, role) => {
    if (!projectId || !member?.userId || role === member.role) return;
    const key = `role:${member.userId}`;
    setBusyKey(key);
    try {
      const res = await websiteApi.updateProjectMemberRole({ projectId, userId: member.userId, role });
  ...
  const removeMember = async (member) => {
    ...
    setBusyKey(key);
    try {
      const res = await websiteApi.removeProjectMember({ projectId, userId: member.userId });
      ...
      if (member.userId === currentUser?.userId) onOpenChange(false);
    } catch (error) {
      toast({
      ...
      <DialogContent className="max-w-2xl border-[var(--stitch-line)] bg-[var(--stitch-surface-strong)] text-[var(--stitch-ink)]">
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <div className="max-h-[42vh] overflow-y-auto rounded-2xl border border-[var(--stitch-line)] bg-[var(--stitch-surface)]">
              {members.map((member) => {
